 ...
 *          microphone frames to a recognized command word and report it,
 *          but nothing about LVGL/app_state, and (as of Mission 12) nothing
 *          about triggering the Mission 10 capture-and-upload state machine
 *          either - see the Mission 12 header comment in voice_control.c for
 *          why that trigger was deliberately disconnected for this pass.
 */
#pragma once
...
typedef enum {
    ...
    VOICE_STATE_COMMAND_WINDOW,     /* MultiNet is listening for one of the six commands */
    ...
    VOICE_STATE_UNRECOGNIZED,       /* held briefly - MultiNet detected a phrase not in the six-command set */
    VOICE_STATE_CAPTURING,          /* unused this pass (Mission 12 does not trigger capture) - kept so this
                                      * enum stays source-compatible with the Mission 11 capture-trigger path */
    VOICE_STATE_DEGRADED,           /* speech pipeline unavailable this boot - manual REC still works */
} voice_state_t;

/* The six recognition-test commands (Mission 12). IDs start at 1, matching
 * esp_mn_commands_add()'s requirement, and double as the array index (id-1)
 * into the UI's six on-screen buttons - see status_deck_ui.c's cmd_defs[]. */
typedef enum {
    VOICE_CMD_NONE = 0,
    VOICE_CMD_SEND = 1,
    VOICE_CMD_NOTE = 2,
