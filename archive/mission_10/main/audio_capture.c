...
void audio_capture_init(audio_cap_event_cb_t cb, void *user_ctx)
{
    ...
    if (!s_capture_buf) {
        ...
        /* bsp_audio_codec_microphone_init() lazily brings up bsp_i2c_init()
         * and bsp_audio_init(NULL) itself if audio hasn't been touched yet -
         * same shared I2C bus the IMU (live_data_deck_init) already uses,
         * not a second bus or invented wiring. */
