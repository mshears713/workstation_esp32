...
/**
 * Allocates the bounded capture buffer, initializes the ES7210 microphone
 * codec device via the verified esp-box-3 BSP path, and starts the
 * persistent worker task. Call once, from any task, after bsp_display_start()
 * (BSP audio init shares the I2C bus with bsp_i2c_init(), already brought up
 * for the display/touch/IMU path). If buffer allocation or codec bring-up
 * fails, this logs the failure and audio_capture_start() will always return
 * false afterward - the rest of the console keeps running regardless.
 */
void audio_capture_init(audio_cap_event_cb_t cb, void *user_ctx);
