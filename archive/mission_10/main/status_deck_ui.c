...
void status_deck_ui(lv_obj_t *scr)
{
     ...
     * Mission 09, then another 16px (64px -> 48px) in Mission 10 to make
     * room for the AUD status row above - the chart is still full width and
     * still shows the same ~10s of history, just visually shorter. */
    chart = lv_chart_create(scr);
    lv_obj_set_size(chart, 304, 48);
    lv_obj_align(chart, LV_ALIGN_TOP_MID, 0, 76);
    ...
    render_audio_panel();
    audio_capture_init(audio_status_changed_cb, NULL);
